...
def demo_message_passing():
    ...
    def create_message_passing_pipeline():
        ...
        def fact_checker(state: MessagePassingState) -> MessagePassingState:
        ...
        def summarizer(state: MessagePassingState) -> MessagePassingState:
            ...
            return {
                "messages": [
                    AIMessage(
                        content=f"[SUMMARY]: {response.content}", name="summarizer")
                ],
                "current_phase": "done",
            }
    ...
    visualize_graph(agent.get_graph(), './agent_comm__message_passing.png')
...
def demo_shared_state():
    ...
    def create_shared_fields_pipeline():
        ...
        def data_collector(state: SharedFieldsState) -> SharedFieldsState:
        ...
        def analyst(state: SharedFieldsState) -> SharedFieldsState:
        ...
        def advisor(state: SharedFieldsState) -> SharedFieldsState:
    ...
    visualize_graph(agent.get_graph(), './agent_comm__shared_state.png')
    ...
    for d in result["raw_data"]:
        print(
            f"  - [{d.get('source', 'N/A')}] {d.get('finding', 'N/A')[:80]}...")
...
def demo_blackboard():
    ...
    def create_blackboard_system():
        ...
        class ApprovalDecision(BaseModel):
            approved: bool = Field(
                description="Whether the draft is good enough")
            feedback: str = Field(
                description="Specific feedback if not approved")

        critic_model = model.with_structured_output(ApprovalDecision)

        def drafter(state: BlackboardState) -> BlackboardState:
            ...
            if state["critiques"]:
                context_parts.append(
                    f"Feedback to address: {state['critiques'][-1]}")
        ...
        def critic(state: BlackboardState) -> BlackboardState:
    ...
    visualize_graph(agent.get_graph(), './agent_comm__backboard.png')
